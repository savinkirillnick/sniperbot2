...
class DB:
    ...
    def query(self, db_name, query_get, query_post=()):
    ...
    def __init__(self):
        self.query('settings.db',
                   'CREATE TABLE IF NOT EXISTS init_settings (id INT PRIMARY KEY, shared_key VARCHAR(16), time INT)')
        self.query('settings.db',
        ...
        if not row:
            self.query('settings.db', 'INSERT INTO init_settings (id, shared_key, time) VALUES (1, "demo", 0)')

    def access(self, access_time):
        self.access_time = access_time

    def save_key(self, shared_key, access_time):
        if self.access_time > time():
            self.query('settings.db', 'UPDATE init_settings SET shared_key=?, time=? WHERE id=?', (shared_key, access_time, 1))
            return 0
